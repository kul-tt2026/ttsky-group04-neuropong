...
async def test_reset_state(dut):
    ...
    assert color == WHITE, f"Ball's top-left corner must be at ({H_DISPLAY // 2},{V_DISPLAY // 2})"
...
async def test_color_bit_mapping(dut):
    ...
    color = await sample_pixel(dut, cursor, L_PADDLE_X + 5, V_DISPLAY // 2 + 10)
    assert color == RED, f"Left paddle pixel should be pure red, got {color}"

    # Right paddle -> pure blue (v=250, h=600 -- same row, higher h)
    color = await sample_pixel(dut, cursor, R_PADDLE_X + 5, V_DISPLAY // 2 + 10)
...
async def test_left_paddle_movement(dut):
    ...
    top_h, top_v = L_PADDLE_X + 5, V_DISPLAY // 2
